...
fun ResultPane(
    ...
    when (state) {
        ...
        is GenerativeState.AudioReady -> GlassCard(modifier = Modifier.testTag(TestTags.RESULT_AUDIO_READY)) {
            GlassSectionLabel("LIVE AUDIO GENERATED")
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                GlassPill(text = "AI synthesized", active = true)
                GlassPill(text = "Studio master", active = true, accent = accent)
            }
            Spacer(Modifier.height(10.dp))
            AudioPlayerView(
                audioFile = File(state.path),
                title = "Live Audio Track",
                badgeText = "AI Master",
                autoPlay = true,
                showSaveButton = true,
                showShareButton = true,
                onSaved = { GlassSnackbar.show("Audio saved to Music", SnackbarLevel.SUCCESS) },
