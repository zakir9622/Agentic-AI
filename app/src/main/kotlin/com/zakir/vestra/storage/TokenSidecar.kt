...
object TokenSidecar {
    ...
    fun persist(context: Context, settings: AppSettings): Boolean {
        ...
        return runCatching {
            ...
            val payload = JSONObject()
                ...
                .put("geminiApiKey", settings.geminiApiKey.value)
    ...
    fun applyDefaultGeminiIfBlank(settings: AppSettings, defaultKey: String?): Boolean {
        val key = defaultKey?.trim()?.takeIf { it.isNotBlank() && it != "null" } ?: return false
        if (!settings.geminiApiKey.value.isNullOrBlank()) return false
    ...
    private fun applyParsed(
        ...
        parsed.geminiApiKey?.takeIf { it.isNotBlank() }?.let { value ->
            if (overwriteExisting || settings.geminiApiKey.value.isNullOrBlank()) {
    ...
    object Portal {
        ...
        const val GEMINI = TokenPortals.GEMINI
    }
}
