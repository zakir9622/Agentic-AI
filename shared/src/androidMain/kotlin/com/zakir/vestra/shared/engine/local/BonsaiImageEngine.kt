 ...
 * kill on 8 GB devices. A run takes several minutes on a Pixel-class CPU; there is no
 * per-step progress callback, matching every other synchronous [LocalImageGenerator].
 *
...
class BonsaiImageEngine(
    ...
    override fun generate(prompt: String, seed: Long?, referenceImageUri: String?): LocalImageResult {
        ...
        val dir = packDir() ?: return LocalImageResult.Unavailable("Local image pack directory missing.")
        val meta = loadMeta(dir)
            ?: return LocalImageResult.Unavailable(
                "Pack pipeline_meta.json missing or invalid — re-download $packId.",
            )
        val missing = missingFiles(dir, meta)
        if (missing.isNotEmpty()) {
            return LocalImageResult.Unavailable(
        ...
        return try {
            packs.markPackInUse(packId)
            runGeneration(dir, meta, prompt, seed ?: System.currentTimeMillis())
        } finally {
            packs.markPackIdle(packId)
        }
    }

    private fun runGeneration(dir: File, meta: PipelineMeta, prompt: String, seed: Long): LocalImageResult =
        runCatching {
