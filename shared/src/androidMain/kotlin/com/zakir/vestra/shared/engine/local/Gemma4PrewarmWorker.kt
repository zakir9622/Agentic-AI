...
/**
 * Background WorkManager worker that pre-warms the on-device Gemma 4 model when the device is
 * charging and connected to an unmetered network. Ported from GoogleLookBookUI, adapted to this
 * repo's [ModelPackManager]/[LiteRtLmInference] call shapes (which carry a few extra vision/audio/
 * NPU/speculative-decoding params beyond what GoogleLookBookUI's fork has — all left at their
 * defaults here since prewarm only needs the plain code-model path).
 *
 * This ensures the heavy weights (~2.6 GB) are cached in memory ahead of time, eliminating the
 * cold-load delay on the user's first prompt.
 */
class Gemma4PrewarmWorker(
    ...
    companion object {
        ...
        suspend fun executePrewarm(
            ...
            Log.i(TAG, "Executing background Gemma 4 pre-warm on unmetered network + charging...")
        ...
        /**
         * Builds the WorkManager constraints ensuring execution only occurs while the device is
         * charging and on an unmetered network.
         */
        ...
        /**
         * Builds the periodic WorkRequest with charging and unmetered-network constraints.
         */
        ...
        /**
         * Builds the one-time WorkRequest with charging and unmetered-network constraints.
         */
         ...
         * with charging and unmetered-network constraints.
         */
        ...
        /**
         * Enqueues a one-time pre-warm request with charging and unmetered-network constraints.
         */
        fun enqueueOneTime(
            context: Context,
            packId: String = LiteRtLmPacks.GEMMA4_CODE,
