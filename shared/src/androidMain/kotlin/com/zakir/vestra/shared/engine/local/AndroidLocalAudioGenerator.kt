...
/**
 * Android TTS pack gate — stays unavailable until [LocalAudioFlags.TTS_RUNNER_WIRED]
 * and real `local-tts-v1` graphs land (same honesty pattern as SD-Turbo).
 */
class AndroidLocalAudioGenerator(
    ...
    override fun isReady(): Boolean {
        if (!LocalAudioFlags.TTS_RUNNER_WIRED) return false
        return packs.isReady(packId)
    }
    ...
    override fun generate(
        ...
        if (!isReady()) {
            return LocalAudioResult.Unavailable(
                "On-device TTS locked (runner=${LocalAudioFlags.TTS_RUNNER_WIRED}, pack=$packId). " +
                    "Use cloud Audio Studio; knobs still apply locally after download.",
            )
        }
        return LocalAudioResult.Unavailable(
            "local-tts-v1 graphs present but sampler not productized — using cloud TTS.",
        )
    }

    companion object {
        const val PACK_ID = "local-tts-v1"
    }
}
