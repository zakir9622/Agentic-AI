...
| [`generation-transparency/`](generation-transparency/) | Claude Code read-only audit (post rc9) | **v3.1.0-rc9** | On-device latency benchmark harness, Pro-pack export collision, OrtSessionCache in diffusion path, per-tab generation log + timer, reduced-motion coverage | Not started — supplements `five-star-quality/`, no new visual system |
