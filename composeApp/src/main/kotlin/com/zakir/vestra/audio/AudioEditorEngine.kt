...
import com.zakir.vestra.shared.audio.CustomVoiceProfile
...
/**
 * Local (on-device) audio decode/trim/vocal-DSP/voice-transform pipeline. Ported from
 * GoogleLookBookUI; WAV I/O delegates to the shared [WavIo] writer instead of duplicating it.
 */
object AudioEditorEngine {
    ...
    fun trimAudio(
        ...
        // Clamp the start first, then re-derive its ceiling from the (now known) total so a
        // start within 50ms of the clip's end doesn't hand coerceIn an empty range to throw on.
        val validStartMs = startMs.coerceIn(0L, (totalMs - 50L).coerceAtLeast(0L))
        ...
        WavIo.writePcm16Wav(outFile, trimmedSamples, track.sampleRate, track.channels)
        return outFile
    }
    ...
    fun processVocals(
        ...
        val track = decodeAudio(inputFile) ?: return null
        // The stereo DSP below is inherently an L/R algorithm (center-channel cancellation etc.);
        // it indexes frames as exactly 2 interleaved samples. A >2-channel file (e.g. surround
        // AAC/M4A) would otherwise have its frame count computed from all channels while indexing
        // only walked 2 at a time, leaving the tail of the output buffer silently zero-filled.
        // Downmix to true stereo (first two channels — the front L/R in standard WAV/AAC channel
        // order) before running the algorithm instead.
        val (samples, channels) = if (track.channels > 2) {
            val frameCount = track.samples.size / track.channels
            val stereo = ShortArray(frameCount * 2)
            for (i in 0 until frameCount) {
                stereo[i * 2] = track.samples[i * track.channels]
                stereo[i * 2 + 1] = track.samples[i * track.channels + 1]
            }
            stereo to 2
        } else {
            track.samples to track.channels
        }
        ...
        WavIo.writePcm16Wav(outFile, processedSamples, sampleRate, channels)
        return outFile
    }
    ...
    fun transformVoicePreset(
        ...
        var monoSamples = downmixToMono(track.samples, track.channels)
        ...
        WavIo.writePcm16MonoWav(outFile, monoSamples, track.sampleRate)
        return outFile
    }
    ...
    fun analyzeVoiceSample(
        ...
        val sampleRate = track.sampleRate
        val monoSamples = downmixToMono(track.samples, track.channels)
    ...
    fun transformToCustomVoice(
        ...
        WavIo.writePcm16MonoWav(outFile, monoSamples, sampleRate)
        return outFile
    }

    /**
     * Averages all [channels] interleaved samples per frame down to mono — generic over channel
     * count, unlike hardcoding a 2-channel (L+R)/2 average that silently drops channels 3+ on a
    ...
    private fun applyPitchAndSpeed(
        ...
        // faster (shorter, faster-sounding output) — matches AndroidLocalVoiceChanger's fix for
        // the same formula (dividing made a 2x "Speed" setting play twice as slow).
    ...
    /**
     * Delegates RIFF/fmt/data chunk-parsing to the shared [WavIo.readAnyWav] (PCM 8/16/24/32-bit
     * and float32, any channel count) instead of duplicating a narrower mono/stereo 16-bit-only
     * parser here.
     */
    private fun readPcm16Wav(file: File): PcmAudioTrack? {
        val raw = WavIo.readAnyWav(file) ?: return null
        if (raw.samples.isEmpty()) return null
        val samples = ShortArray(raw.samples.size) { i ->
            (raw.samples[i] * 32767f).roundToInt().coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt()).toShort()
        }
        val durationMs = ((samples.size.toLong() * 1000L) / (raw.sampleRate.toLong() * raw.channels)).coerceAtLeast(1L)
        return PcmAudioTrack(samples, raw.channels, raw.sampleRate, durationMs)
    }
