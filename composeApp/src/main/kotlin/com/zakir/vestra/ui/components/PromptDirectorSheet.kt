...
/**
 * Structured "creative brief" helper — optional scaffolding for building a prompt out of
 * subject/setting/mood/etc fields plus deterministic style-modifier clauses ([StyleModifier]),
 * previewed live via [PromptExpander.expand]. Nothing reaches the composer until "Apply to
 * prompt" is tapped.
 *
 * Ported from GoogleLookBookUI as-is: not called from any screen there either (no entry point —
 * e.g. a "Recipe helper" button next to the prompt composer — has been designed for it upstream).
 * Left unwired here for the same reason rather than inventing that entry point unilaterally.
 */
